...
class GroundingLoss(nn.Module):
    ...
    def _grounding_loss(self, cls_emb_pred: torch.Tensor, gt_caption_embs: torch.Tensor, gt_caption_mask: float, temperature: float) -> float:
        ...
        return (1 - self._sparsity_loss_relative_weight) * grounding_loss + \
            self._sparsity_loss_relative_weight * GroundingLoss._v2l_sparsity_loss(att_v2l=attention_v2l, batch_size=batch_size)
    ...
    def _v2l_sparsity_loss(att_v2l: torch.Tensor, batch_size: int, target_mean: float=0.05) -> float:
        ...
        rho = inner_image_att_v2l.mean(dim=0)  # [Ntok, N_q]
